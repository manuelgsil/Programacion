...
public class Ejercicio07_Vuelos {
	...
	private static void preguntarUsuario(int[] asientos) {
		...
		do { // Este bucle se ejecutará hasta que el valor de eleccion sea FALSE.
			if (eleccion) {
				comprobarAsientos(asientos); // Se comprobará la disponibilidad de los asientos y empieza una llamada
												// anidada
				System.out.println("Quiere reservar otra plaza?"); // Una vez que se termine la anidación se le
																	// preguntará si quiere otro billete
	 ...
	 * @param eleccion valor booleano que retornaremos y que está previamente
	 *                 cargado con False.
	 ...
	 * 
	 * El objetivo principal de esta función es identificar el numero de asientos
	 * libres, lo que nos permitirá crear una nueva array con el tamaño exacto de
	 ...
	 * 
	 *      Dicho array será lo que mostraremos al usuario a traves de otra llamada
	 *      a otro metodo:
	 * 
	 ...
	 * {@value posicion} que nos servirá como punto de referencia para movernos en
	 * el array a generar.
	 * 
	 ...
	 * opción del usuario a través del uso de un IF anidado. Contiene una llamada a
	 * metodo @see {@link #asignarAsiento(int[])} que nos permitirá asignar la
	 * posición indicada por el usuario en el ARRAY ORIGINAL. Una vez este metodo
	 ...
	 * índice de 0 a 1 segun la plaza seleccionada por el usuario. SI no fuera así,
	 * tendría que escoger otro asiento.
	 * 
	 * @param asientosAvion recibe el array original y asignamos mediante un bucle
	 *                      for y una condición la plaza si es que está disponible
	 */
